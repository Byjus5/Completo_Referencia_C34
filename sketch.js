...
function draw() {
  ...
  //Condición para verificar si toca alguna cosa mala
  if(badGroup.isTouching(player)){
    player.x=300;
    player.y=200;
    chance();
  }
...
//Hacer colisionar al jugador con los bordes
  player.collide(upperWall);
  player.collide(lowerWall);
  player.collide(leftWallup);
  player.collide(leftWalldown);
  player.collide(rightWallup);
  player.collide(rightWalldown);
...
if(player.x === 0 && (player.y >=180 || player.y <= 220)){
  ...
  text("Fin del juego", 250, 200);
    text("¡Bien hecho Ganaste!", 2000, 250);
  ...
  //Estado del juego "OVER"
  if(gameState === "OVER"){
    badGroup.destroyEach();
    textSize(25);
  fill("yellow");
    text("¡Fin del juego!", 230, 200);
    text("Presiona tecla R para reiniciar", 150, 250);
    restart();
...
function treasure(){
  ...
  function lifes(){
    life1= createSprite(380, 20, 10,10);
    life1.addImage("life1", lifeImg);
    life1.scale =0.5;
    life2= createSprite(400, 20, 10,10);
    life2.addImage("life2", lifeImg);
    life2.scale =0.5;
    life3= createSprite(420, 20, 10,10);
